...
class DefaultSchemaMergerTest {
    ...
    //     // 测试合并
    //     List<CsdlSchema> schemasToMerge = Arrays.asList(baseSchema, extensionSchema);
    //     SchemaMerger.MergeResult result = merger.mergeSchemas(schemasToMerge);

    //     // 验证合并结果
    //     assertNotNull(result);
    //     assertTrue(result.isSuccess());
    //     assertNotNull(result.getMergedSchemas());
    //     assertEquals(2, result.getMergedSchemas().size()); // 两个不同的namespace

    //     // 验证Base namespace保持独立
    //     boolean foundBase = result.getMergedSchemas().stream()
    //         .anyMatch(s -> "Base".equals(s.getNamespace()));
    //     assertTrue(foundBase);

    //     // 验证Extension namespace保持独立
    //     boolean foundExtension = result.getMergedSchemas().stream()
    //         .anyMatch(s -> "Extension".equals(s.getNamespace()));
    ...
    //     // 测试合并所有三个Schema
    //     List<CsdlSchema> allSchemas = Arrays.asList(productsSchema, salesSchema, inventorySchema);
    //     SchemaMerger.MergeResult result = merger.mergeSchemas(allSchemas);

    //     // 验证合并结果
    //     assertNotNull(result);
    //     assertTrue(result.isSuccess());
    //     assertNotNull(result.getMergedSchemas());
    //     assertEquals(3, result.getMergedSchemas().size()); // 三个不同的namespace

    //     // 验证每个namespace都被保留
    //     Set<String> namespaces = new HashSet<>();
    //     for (CsdlSchema schema : result.getMergedSchemas()) {
    //         namespaces.add(schema.getNamespace());
    //     }

    //     assertTrue(namespaces.contains("Products"));
    //     assertTrue(namespaces.contains("Sales"));
    //     assertTrue(namespaces.contains("Inventory"));

    //     // 验证每个Schema的内容完整性
    //     for (CsdlSchema schema : result.getMergedSchemas()) {
    //         assertNotNull(schema.getEntityContainer());
    //         if ("Products".equals(schema.getNamespace())) {
    //             assertEquals(1, schema.getEntityTypes().size());
    //             assertEquals(1, schema.getComplexTypes().size());
    //             assertEquals("Product", schema.getEntityTypes().get(0).getName());
    //             assertEquals("Category", schema.getComplexTypes().get(0).getName());
    //         } else if ("Sales".equals(schema.getNamespace())) {
    //             assertEquals(1, schema.getEntityTypes().size());
    //             assertEquals(1, schema.getEnumTypes().size());
    //             assertEquals("Sale", schema.getEntityTypes().get(0).getName());
    //             assertEquals("SaleStatus", schema.getEnumTypes().get(0).getName());
    //         } else if ("Inventory".equals(schema.getNamespace())) {
    //             assertEquals(1, schema.getEntityTypes().size());
    //             assertEquals(1, schema.getComplexTypes().size());
    //             assertEquals("InventoryItem", schema.getEntityTypes().get(0).getName());
    //             assertEquals("WarehouseLocation", schema.getComplexTypes().get(0).getName());
    //         }
    //     }
    // }
