...
/**
 * Test class for Function merging scenarios in DefaultSchemaMerger
 */
@DisplayName("Function Merger Tests")
public class DefaultSchemaMergerFunctionTest {
    ...
    @Test
    @DisplayName("Function KEEP_FIRST - Should keep first definition")
    void testFunction_KeepFirst() throws Exception {
        // Load test schemas
        CsdlSchema schema1 = XmlSchemaTestUtils.loadSchemaFromXml("merger/function/keep-first/schema1.xml");
        CsdlSchema schema2 = XmlSchemaTestUtils.loadSchemaFromXml("merger/function/keep-first/schema2.xml");
        ...
        assertNotNull(mergedSchema.getFunctions(), "Merged schema should have functions");
        assertEquals(1, mergedSchema.getFunctions().size(), "Should have one function");
        
        // Check that first schema's function is preserved
        assertEquals("GetTotalPrice", mergedSchema.getFunctions().get(0).getName());
        assertEquals(2, mergedSchema.getFunctions().get(0).getParameters().size(), "Should have 2 parameters from first schema");
        assertEquals("Edm.Decimal", mergedSchema.getFunctions().get(0).getReturnType().getType(), "Return type should be Decimal from first schema");
    }
    
    @Test
    @DisplayName("Function KEEP_LAST - Should keep last definition")
    void testFunction_KeepLast() throws Exception {
        // Load test schemas
        CsdlSchema schema1 = XmlSchemaTestUtils.loadSchemaFromXml("merger/function/keep-last/schema1.xml");
        CsdlSchema schema2 = XmlSchemaTestUtils.loadSchemaFromXml("merger/function/keep-last/schema2.xml");
        ...
        // Check that last schema's function is preserved
        assertEquals("GetTotalPrice", mergedSchema.getFunctions().get(0).getName());
        assertEquals(2, mergedSchema.getFunctions().get(0).getParameters().size(), "Should have 2 parameters from last schema");
        assertEquals("Edm.Decimal", mergedSchema.getFunctions().get(0).getReturnType().getType(), "Return type should be Decimal from last schema");
    }
    
    @Test
    @DisplayName("Function THROW_ERROR - Should throw error on conflict")
    void testFunction_ThrowError() throws Exception {
        // Load test schemas
        CsdlSchema schema1 = XmlSchemaTestUtils.loadSchemaFromXml("merger/function/keep-first/schema1.xml");
        CsdlSchema schema2 = XmlSchemaTestUtils.loadSchemaFromXml("merger/function/keep-first/schema2.xml");
    ...
    @Test
    @DisplayName("Function SKIP_CONFLICTS - Should skip conflicting definitions")
    void testFunction_SkipConflicts() throws Exception {
        // Load test schemas
        CsdlSchema schema1 = XmlSchemaTestUtils.loadSchemaFromXml("merger/function/keep-first/schema1.xml");
        CsdlSchema schema2 = XmlSchemaTestUtils.loadSchemaFromXml("merger/function/keep-first/schema2.xml");
        ...
        // Verify conflicting function is skipped
        CsdlSchema mergedSchema = result.getMergedSchemas().get(0);
        assertTrue(mergedSchema.getFunctions() == null || mergedSchema.getFunctions().isEmpty(), 
                  "Should have no functions due to conflict skipping");
    }
    
    @Test
    @DisplayName("Function AUTO_MERGE - Should report cannot auto-merge")
    void testFunction_AutoMerge() throws Exception {
        // Load test schemas
        CsdlSchema schema1 = XmlSchemaTestUtils.loadSchemaFromXml("merger/function/keep-first/schema1.xml");
        CsdlSchema schema2 = XmlSchemaTestUtils.loadSchemaFromXml("merger/function/keep-first/schema2.xml");
        ...
        // Verify failure (functions cannot be auto-merged)
        assertFalse(result.isSuccess(), "Merge should fail as functions cannot be auto-merged");
        assertFalse(result.getErrors().isEmpty(), "Should have errors about inability to auto-merge");
        assertFalse(result.getConflicts().isEmpty(), "Should have conflict information");
    }
    
    @Test
    @DisplayName("Function No Conflict - Should merge without issues")
    void testFunction_NoConflict() throws Exception {
        // Create schema with unique function name
        CsdlSchema schema1 = XmlSchemaTestUtils.loadSchemaFromXml("merger/function/keep-first/schema1.xml");
