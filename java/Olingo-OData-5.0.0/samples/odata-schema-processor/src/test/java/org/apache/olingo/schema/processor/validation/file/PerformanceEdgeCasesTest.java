...
public class PerformanceEdgeCasesTest {
    ...
    private static final String PERFORMANCE_EDGE_CASES_DIR = "src/test/resources/validator/file/09-performance-edge-cases";
    ...
    public void testManyReferences() {
        ...
        long validationTime = System.currentTimeMillis() - startTime;
        assertNotNull(result, "Result should not be null");
        boolean foundRefError = result.getErrors().stream().anyMatch(e ->
            e.contains("reference") || e.contains("Reference") ||
            e.contains("引用过多") || e.contains("too many references") ||
            e.contains("Failed to load Reference") || e.contains("loading failed"));
